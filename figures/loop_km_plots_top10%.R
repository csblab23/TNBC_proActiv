...
#survival plot for promoters

# file1 = "/home/kirti/proActiv/univariate_surv_proActiv/10_counts_rpa_updated/10%_union_60/com_p_g_sig_pv.csv"
# list1 = read_csv(file1)
# dim(list1)
# gene_names = list1$x #"ENSG00000005194.15" "ENSG00000136044.12" "ENSG00000143624.14"
...
for (sample_col in sample_col_list) {
  test_surv_df <- data.frame(Run = prom_matrix$Run, Event = prom_matrix$RFS_Status, Time = prom_matrix$RFS_time_Days)
  mm_check_subset <- prom_matrix[, c('Run', sample_col)]
  mm_check_subset <- mm_check_subset %>% arrange(desc(!!sym(sample_col)))
  ...
  sfit <- survfit(Surv(merged_df$Time, merged_df$Event)~ merged_df[,'Binary'], data=merged_df)
...
#similarly for plotting survival plots for gene expression
for (sample_col in gene_names) {
  test_surv_df <- data.frame(Run = prom_matrix$Run, Event = prom_matrix$RFS_Status, Time = prom_matrix$RFS_time_Days)
  mm_check_subset <- gene_matrix[, c('Run', sample_col)]
  mm_check_subset <- mm_check_subset %>% arrange(desc(!!sym(sample_col)))
  mm_check_subset <- mm_check_subset %>%
    mutate(Binary = ifelse(row_number() <= round(dim(mm_check_subset)[1]*0.1), "high", "low"))
  merged_df <- merge(test_surv_df, mm_check_subset, by = "Run")
  ...
  sfit <- survfit(Surv(merged_df$Time, merged_df$Event)~ merged_df[,'Binary'], data=merged_df)
  plot_title <- paste0("Survival Plot for ", sample_col)
  p = ggsurvplot(sfit, pval=TRUE, title= plot_title, legend.labs=c("HIGH", "LOW"), legend.title="", palette = c("royalblue", "magenta"))
  plot_object <- p$plot
  filename <- paste0("plot_", gsub(" ", "_", sample_col), ".pdf")
  ggsave(filename, plot = plot_object, device = "pdf")
}
